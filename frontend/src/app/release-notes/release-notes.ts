  ...
  isInputValid(): boolean {
    if (this.inputMode === 'kb') {
      if (this.selectedInputType === 'BRD + Codebase') {
        return !!(this.selectedProjectId && this.selectedBrdId && this.selectedCodebaseId);
      }
      return !!(this.selectedProjectId && this.selectedDocumentId);
    }
    return !!this.sprintInformation.trim();
  }

  getModalMeta() {
    return {
      project: this.selectedProject?.projectName || undefined,
      documentName: this.selectedDocument ? this.selectedDocument.fileName : 'Release Notes',
      ...
      version: this.selectedBrdDocument?.version || this.selectedDocument?.version || undefined,
  ...
  generate() {
    if (!this.isInputValid()) {
      this.error = this.inputMode === 'kb' 
        ? 'Please select a Project and Document.'
        : 'Please enter sprint information.';
      this.cdr.markForCheck();
      return;
    }
    ...
    const sprintDetails = this.inputMode === 'kb'
      ? `Release Notes based on ${this.selectedInputType === 'BRD + Codebase'
